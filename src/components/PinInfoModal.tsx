		...
		<div className="absolute bottom-0 left-0 w-full z-10 bg-white rounded-t-xl shadow-lg p-4">
			...
			<div className="flex items-center justify-center gap-4 mt-4">
				<button 
                    ...
                    onMouseEnter={() => {if (!isSaved) setIsSaveHovered(true);}}
                    onMouseLeave={() => {if (!isSaved) setIsSaveHovered(false);}}    
                    className={`flex items-center justify-center gap-2 px-3 rounded-lg py-1.5 text-sm 
                        ...
                        : "bg-[#D9D9D91A] text-[#8F8F8F] hover:text-[#FFAC33] hover:border-[#FFAC33] border border-transparent cursor-pointer"} transition`}
                    style={{
                        boxShadow: isSaved
                    ...
                    <SavePlaceIcon style={{color: isSaved ? "#FFAC33" : isSaveHovered ? "#FFAC33" : "#D9D9D9"}} />
					{isSaved ? "저장됨" : "내 장소 저장"}
				</button>
				<button
